...
import "./style.css";
import explosionImg from "./explosion.png";
...
const createPieces = () => {
  ...
  for (let i = 1; i < amountOfPieces + 1; i++) {
...
const placePiecesAndShuffle = () => {
  ...
  const placeCorrectPieces = () => {
    ...
    for (let i = Math.round(shuffled.length / 5); i < shuffled.length; i++) {
  ...
  const placeBombs = () => {
    let bombs = [];
    for (let o = -1; o < shuffled.length / 5; o++) {
